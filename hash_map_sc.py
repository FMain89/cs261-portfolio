...
class HashMap:
    ...
    def put(self, key: str, value: object) -> None:
        """
        Description:
            Method takes a key and a value as parameters and updates the
            hash map accordingly. If the key already exists, the value
            is updated. If the passed key is new, a new key/value pair is
            added.
    ...
    def empty_buckets(self) -> int:
        ...
        for bucket_iterator in range(self._capacity):
            current_bucket = self._buckets[bucket_iterator]
    ...
    def resize_table(self, new_capacity: int) -> None:
        ...
        Description:
            ...
            prime. All existing key/value pairs remain in the new hash map
            and all hash tables links are rehashed.
        ...
        prime_test = self._is_prime(new_capacity)
        if not prime_test:
        ...
        for bucket in range(original_capacity):
            current_bucket = self._buckets.get_at_index(bucket)
            for node in current_bucket:
                key = node.key
                value = node.value

                hashed_key = self._hash_function(key) % new_capacity
    ...
    def get(self, key: str):
        ...
        Description:
            ...
            has map, the method returns None.
        ...
        Returns:
            value: value in hash map associated with passed key.
        """
    ...
    def remove(self, key: str) -> None:
        ...
        Description:
            ...
            currently stored in the hash map. If the key is not in the has
    ...
    def get_keys_and_values(self) -> DynamicArray:
        """
        Method takes no parameters and returns a Dynamic Array of all the keys
        and values stored in the hash map. The returned Dynamic Array does not
        have any specified order or sorting.
        ...
        for hashed_key in range(self._capacity):
            current_bucket = self._buckets.get_at_index(hashed_key)
            for node in current_bucket:
...
def find_mode(da: DynamicArray) -> tuple[DynamicArray, int]:
    """
    Function takes a DynamicArray and returns a tuple containing a
    DynamicArray of the mode(s) and the frequency of the mode(s).

    Parameters:
        da: DynamicArray containing the values to find the mode(s) of.

    Returns:
        (modes, max_frequency): A tuple where modes is a DynamicArray of the
        mode(s) and max_frequency is the frequency of the mode(s).
    """
    # Use a HashMap to count the frequency of each value in the DynamicArray
    frequency_map = HashMap()

    for i in range(da.length()):
        value = da.get_at_index(i)
        if frequency_map.contains_key(value):
            frequency = frequency_map.get(value)
            frequency_map.put(value, frequency + 1)
        else:
    ...
    max_frequency = 0
    modes = DynamicArray()
    keys_values = frequency_map.get_keys_and_values()

    for i in range(keys_values.length()):
        value, frequency = keys_values.get_at_index(i)
